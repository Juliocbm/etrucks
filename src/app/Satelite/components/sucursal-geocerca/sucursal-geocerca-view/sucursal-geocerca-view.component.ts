...
import { Sucursal, SucursalGeocerca} from 'src/app/models/RH/sucursal';
...
import { ModalCrudSucursalGeocercaComponent } from '../modal-crud-sucursal-geocerca/modal-crud-sucursal-geocerca.component';


@Component({
  selector: 'app-sucursal-view',
  templateUrl: './sucursal-geocerca-view.component.html',
  styleUrls: ['./sucursal-geocerca-view.component.css']
})
export class SucursalGeocercaViewComponent implements OnInit {
  showAlert = false;
  alertMessage = '';
  alertType = '';
  ...
  retriveData(){
    ...
    this.apiRhService.obtenerSucursales().subscribe(
      response => {
        ...
        this.datosFiltrados = this.datos.filter(sucursal => 
          sucursal.sucursalDetalle.some(detalle => 
            detalle.idTipoSucursal === 'a6f2769a-273d-4c83-b253-ea58f3b2bff5'
          )
        );
  ...
  onDetailClick(rowData: any) {
    ...
    const dialogRef = this.dialog.open(ModalCrudSucursalGeocercaComponent, {
      width: '1000px',
