import { Component, ViewChild } from '@angular/core';
import { RolModulos } from './../../../../models/Administrador/Menu';
import { ApiAdministradorService } from '../../../../DataAccess/api-administrador.service';
import { ColumnConfig } from '../../../../shared-module/Interfaces/ColumnConfig';
import { TableAction } from './../../../../shared-module/Interfaces/TableAction';
import { TableConfig } from './../../../../shared-module/Interfaces/TableConfig';
import { StorageService } from '../../../../Services/StorageService';
import { ModalCrudRolComponent } from '../modal-crud-rol/modal-crud-rol.component';
import { MatDialog } from '@angular/material/dialog';
import { ModalService } from '../../../../Services/Modal.service';
import { finalize, tap, map } from 'rxjs';
import { AlertService, Alert } from '../../../../Services/alerts.service';
import { FullTableV2Component } from '../../../../shared-module/components/full-tableV2/full-table.component';
import { AuthService } from '../../../../security/services/auth.service';

@Component({
  selector: 'app-catalogo-rol',
  templateUrl: './catalogo-rol.component.html',
  styleUrls: ['./catalogo-rol.component.css'],
})
export class CatalogoRolComponent {
  showAlert = false;
  alertMessage = '';
  alertType = '';
  datosFiltrados: RolModulos[] = [];
  datos: RolModulos[] = [];
  isLoading: boolean = false;
  observableRoles;
  @ViewChild(FullTableV2Component) tableComponent!: FullTableV2Component;
  MENU_ACTUAL: any;

  constructor(
    private apiAdminAccess: ApiAdministradorService,
    private storageService: StorageService<RolModulos>,
    public dialog: MatDialog,
    private modalService: ModalService,
    private alertService: AlertService,
    private authService: AuthService
  ) {
    this.observableRoles = this.apiAdminAccess.obtenerRoles.bind(this.apiAdminAccess);
    this.MENU_ACTUAL = this.authService.getMenuByCveMenu('ROLES');
  }

  columnConfigs: { [key: string]: ColumnConfig } = {
    idRol: {
      displayName: 'Id', 
      type: 'number', 
      showFilter: true, 
      visible: true,
      char: 'Id', 
    },
    nombre: {
      displayName: 'Nombre',
      type: 'default',
      showFilter: true,
      visible: true,
    },
    descripcion: {
      displayName: 'Descipción',
      type: 'default',
      showFilter: true,
      visible: true,
    },
    activo: {
      displayName: 'Estatus',
      type: 'boolean',
      trueValue: 'Activo',
      falseValue: 'Inactivo',
      visible: true,
    },
    fechaModificacion: {
      displayName: 'Modificado',
      type: 'date',
      format: 'dd/MMM/yyyy',
      showFilter: true,
      startDate: null,
      endDate: null,
      visible: true,
      widthColumn: '120px'
    },
    usuarioModificadoPor: {
      displayName: 'Modificado por',
      type: 'default',
      showFilter: true,
      visible: true,
      widthColumn: '120px'
    },
  };

  tableActions: TableAction[] = [
    {
      name: 'ver',
      title: 'Ver',
      icon: 'visibility',
      tooltip: 'Ver',
      callback: (item) => this.obtenerRolPorId(item, 0),
       
    },
    {
      name: 'edit',
      title: 'Editar',
      icon: 'mode_edit',
      tooltip: 'Editar',
      callback: (item) => this.obtenerRolPorId(item, 1),
      showCondition: (item) => item.activo,
    },
    {
      name: 'eliminar',
      title: 'Eliminar',
      icon: 'delete',
      tooltip: 'Eliminar',
      callback: (item) => this.onEliminarClick(item),
      showCondition: (item) => item.activo,
      permission: 'Eliminar',
    }
  ];

  tableConfigs: TableConfig = {
    pageSizeOptions: [5, 15, 30],
    headerColumFontSize: 5,
    createCallback: () => this.onCreateClick(),
  };

  ngOnInit() {
    this.storageService.init('rolActual');
  }

  retriveData = () => {
    this.tableComponent.loadData();
  };

  cambiarEstado(item: any) {}

  onDetailClick(rowData: any)  {
    this.modalService.openModal(
      ModalCrudRolComponent,
      rowData,
      'DETAIL',
      () => this.tableComponent.loadData(),
      this.MENU_ACTUAL.menu,
      '1000px',
      '800px'
    );
  }

  onEditClick(rowData: any) {
    this.modalService.openModal(
      ModalCrudRolComponent,
      rowData,
      'EDIT',
      () => this.tableComponent.loadData(),
      this.MENU_ACTUAL.menu,
      '1000px',
      '800px'
    );
  }


  onCreateClick() {
    this.modalService.openModal(
      ModalCrudRolComponent,
      null,
      'CREATE',
      this.retriveData,
      this.MENU_ACTUAL.menu,
      '1000px',
      '800px',
    );
  }

  onEliminarClick(rowData: any) {
    this.alertService
      .question('¿Eliminar registro de Rol: ' + rowData.nombre + '?')
      .pipe(
        map((isConfirmed) => {
          if (isConfirmed) {
            //Llama al método del servicio para actualizar
            this.isLoading = true; // Muestra el indicador de carga

            const dataForm = rowData as RolModulos; //convertimos el formulario a el tipo de dato necesario

            dataForm.activo = false;
            // Llama al método del servicio para actualizar
            this.apiAdminAccess
              .updEstatusRol(dataForm.id)
              .pipe(
                tap((data) => {
                  this.alertService.success('default.success.eliminado');
                  this.retriveData();
                }),
                finalize(() => {
                  this.isLoading = false;
                })
              )
              .subscribe();
          } else {
            this.alertService.warning('default.warning.cancelar');
          }
        })
      )
      .subscribe();
  }

  obtenerRolPorId(rowData: any, accion: number) {
    this.apiAdminAccess
      .obtenerRolById(rowData.idRol)
      .pipe(
        tap((response) => {
          if (accion == 0) this.onDetailClick(response);
          else this.onEditClick(response);
        }),
        finalize(() => {})
      )
      .subscribe();
  }
  
}
