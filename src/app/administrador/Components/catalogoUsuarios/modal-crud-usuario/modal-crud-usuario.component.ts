...
@Component({
  selector: 'app-modal-crud-usuario',
  templateUrl: './modal-crud-usuario.component.html',
  styleUrls: ['./modal-crud-usuario.component.css'],
})
export class ModalCrudUsuarioComponent {
  ...
  tableConfigsRol: TableConfig = {
   ...
   tableConfigsCatalogoRol: TableConfig = {
    pageSizeOptions: [8],
    headerColumFontSize: 5,
    heightRow: 'auto',
  };
  ...
  constructor(
    ...
    public modal: MatDialogRef<ModalCrudUsuarioComponent>,
  ...
  asignarRol(data: any, compActual: boolean = true): void {
    ...
    this.rolListado = this.rolListado.filter((r) => r.idRol != data.idRol);
  ...
  onSubmit() {
    ...
    this.apiAdminAccess
      ...
      this.apiAdminAccess
        .postUsuario(dataForm.idSistema, dataForm)
        .pipe(
          finalize(() => {
